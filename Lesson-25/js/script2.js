...
next(){
	if(this.currentIndex < this.productList.length){
		...
		return {
			...
			value:`${currentProduct.title} - ${currentProduct.totalProductPrice}`
		}
	}
...
class Product{
	constructor(title,price,quantity){
		...
		this.totalProductPrice = this.getProductTotalPrice()

	}
	getProductTotalPrice(price, quantity){
		price = this.price
	...
	[Symbol.toPrimitive](hint){
		switch (hint) {
			case 'string':
				return `${this.title}-${this.totalProductPrice}`
			default:
				return this.title
		}
	}
